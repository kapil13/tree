...
echo "  2. alembic revision id too long (varchar 32): ensure head is 0013_work_area_bio_snap"
echo "  3. alembic duplicate table: check logs before stamping head"
echo "  4. rebuild: docker compose -f $COMPOSE_FILE --env-file $ENV_FILE up -d --build backend"
echo "  5. manual migrate: docker compose exec backend alembic upgrade head"
