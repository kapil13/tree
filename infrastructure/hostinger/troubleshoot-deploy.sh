...
echo "  6. alembic duplicate table: check logs before stamping head"
echo "  7. rebuild: docker compose -f $COMPOSE_FILE --env-file $ENV_FILE up -d --build backend"
echo "  8. manual migrate: docker compose exec backend alembic upgrade head"
