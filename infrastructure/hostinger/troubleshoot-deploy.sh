...
echo "  2. alembic revision id too long (varchar 32): ensure head is 0017_compliance_checklists"
...
echo "  5. alembic duplicate table: check logs before stamping head"
echo "  6. rebuild: docker compose -f $COMPOSE_FILE --env-file $ENV_FILE up -d --build backend"
echo "  7. manual migrate: docker compose exec backend alembic upgrade head"
