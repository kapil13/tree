...
from app.services.contact.inquiry import deliver_contact_inquiry
...
def test_contact_inquiry_success():
    with patch(
        "app.api.v1.contact.deliver_contact_inquiry",
...
@pytest.mark.asyncio
async def test_deliver_contact_inquiry_dev_fallback():
    payload = ContactInquiryCreate(**VALID_PAYLOAD)
    with patch("app.services.contact.inquiry.resend_configured", return_value=False):
