...
export default function CarbonCalculatorPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <Link href="/settings" className="mb-3 inline-flex items-center gap-1 text-sm text-stone-600 hover:text-forest-800">
          <ArrowLeft className="h-4 w-4" />
          Back to settings
        </Link>
        <h1 className="flex items-center gap-2 text-2xl font-semibold">
          <Calculator className="h-7 w-7 text-forest-700" />
          Carbon calculator
        </h1>
        <p className="mt-1 text-sm text-stone-600">
          Estimate biomass, stored carbon, CO₂e, and credit potential using the same engine as tree AI analysis.
          Results are indicative — field verification is required for issuance.
        </p>
      </div>

      <div className="card grid gap-4 md:grid-cols-2">
        <div className="md:col-span-2">
          <label className="label">Species (scientific or common name)</label>
          <input className="input w-full" value={species} onChange={(e) => setSpecies(e.target.value)} />
        </div>
        <div>
        ...
        <div>
          ...
          <input className="input w-full" type="number" min={0} step="0.1" value={height} onChange={(e) => setHeight(e.target.value)} />
        </div>
        <div>
          <label className="label">Age (years)</label>
          <input className="input w-full" type="number" min={0} step="0.5" value={age} onChange={(e) => setAge(e.target.value)} />
        </div>
        <div>
          <label className="label">Methodology</label>
          <select className="input w-full" value={methodology} onChange={(e) => setMethodology(e.target.value as typeof methodology)}>
        ...
        <div>
          ...
          <input className="input w-full" type="number" min={0} step="0.5" value={price} onChange={(e) => setPrice(e.target.value)} />
        </div>
        <div className="md:col-span-2">
          <button
            type="button"
      ...
      {estimate.error && (
        <div className="rounded-md bg-rose-50 px-3 py-2 text-sm text-rose-700">{errorMessage(estimate.error)}</div>
      )}
      ...
      {result && (
        <div className="card grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="sm:col-span-2 lg:col-span-3 text-xs text-stone-500">
            Engine {result.engine_version} · {result.methodology} · confidence {(result.confidence * 100).toFixed(0)}%
