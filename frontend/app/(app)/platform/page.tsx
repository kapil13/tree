...
const QUICK_LINKS = [
  { href: "/platform/users", labelKey: "manageUsers" as const, icon: Users },
  { href: "/platform/organizations", labelKey: "organizations" as const, icon: Building2 },
  { href: "/platform/program-access", labelKey: "programQueue" as const, icon: ClipboardList },
  { href: "/platform/audit", labelKey: "activityLog" as const, icon: ScrollText },
  { href: "/platform/governance", labelKey: "governance" as const, icon: Shield },
];

export default function PlatformOverviewPage() {
  const t = useTranslations("platformAdmin.overview");
      ...
      {isLoading || !data ? (
        ...
        <div className="space-y-6">
                ...
                {pendingAccess > 0 ? (
                  <li>
                    <Link href="/platform/program-access" className="font-medium underline-offset-2 hover:underline">
                      {t("pendingAccess", { count: pendingAccess })}
                    </Link>
                  </li>
                ) : null}
                {failedJobs > 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              icon={ClipboardList}
              label={t("pendingProgramRequests")}
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-2xl border border-stone-200 bg-white p-5 dark:border-stone-800 dark:bg-stone-900">
              ...
              {auditLoading ? (
                <p className="text-sm text-stone-500">{t("loadingActivity")}</p>
                ...
                <ul className="divide-y divide-stone-100 dark:divide-stone-800">
                  {recentAudit?.items.map((entry) => {
                    const actor =
                      entry.actor_email || entry.actor_full_name || entry.actor_user_id || t("system");
                    return (
