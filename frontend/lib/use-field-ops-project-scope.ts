...
import {
  planFieldOpsScopeSync,
  resolveFieldOpsProjectName,
} from "@/lib/field-ops-project-scope";
import { useProjectContext } from "@/lib/project-context";

export function useFieldOpsProjectScope() {
  ...
  const projectName = useMemo(
    () => resolveFieldOpsProjectName(projectId, projects, selectedProject),
  ...
  useEffect(() => {
    ...
    const plan = planFieldOpsScopeSync(projectIdFromUrl, contextProjectId);
