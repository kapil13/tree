...
import { canAttestMeasurements, canWriteInApp, isVerifier } from "@/lib/nav-access";
...
export function ProjectVerificationPanel({ projectId }: { projectId: string }) {
  ...
  const canCreateSample = canWriteInApp(user) && !isVerifier(user);
  const canAttest = canAttestMeasurements(user);
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-stone-500">
        Supervisors create a sample (random or species-stratified). Verifiers attest trees on the
        verification queue without editing measurements.
      </p>
      {canCreateSample ? (
        ...
        <p className="text-xs text-stone-500">
          Sample creation is limited to supervisors. Open the verification queue to attest pending
          trees.
        </p>
      )}
